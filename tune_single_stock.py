...
try:
    ...
    # from stable_baselines3.common.monitor import Monitor # Optional for detailed logging
except ImportError:
...
class Stock_API:
    ...
    def Get_User_Stocks(self): # Minimal usage in training env usually
...
class StockTradingEnv(gym.Env):
    ...
    def __init__(self, stock_code, start_date, end_date, api_account, api_password,
        ...
        if self.data_df is None or len(self.data_df) < self.window_size: raise ValueError(f"股票 {stock_code} 數據不足")
    ...
    def step(self, action):
        ...
        realized_pnl, cost_basis_before_sell = 0, 0
        if not terminated:
        ...
        else: self.portfolio_value = self._calculate_portfolio_value(self.current_step - 1); close_price_T1 = self.data_df.iloc[self.current_step - 1]['close']
        reward = 0.0
        ...
        if realized_pnl > 0: relative_pnl = realized_pnl / previous_portfolio_value if previous_portfolio_value != 0 else 0; reward += relative_pnl * self.profit_bonus_factor
        elif realized_pnl < 0: relative_pnl = realized_pnl / previous_portfolio_value if previous_portfolio_value != 0 else 0; reward += relative_pnl * self.loss_penalty_factor
...
if __name__ == '__main__':
    ...
    LOSS_PENALTY_FACTOR_TRAIN = 0.4    # 實現虧損的懲罰
    ...
    if RUN_TRAINING:
        ...
        if 'StockTradingEnv' not in globals() or not hasattr(StockTradingEnv, 'step'):
            ...
            try:
                ...
                # 可選: 使用 Monitor 包裹 Env 以記錄更詳細的訓練信息到 TensorBoard
                ...
                # 需要創建一個單獨的評估環境實例
                # eval_env = StockTradingEnv(...) # 使用不同的日期或相同的日期
                # eval_env = Monitor(eval_env, os.path.join(TENSORBOARD_LOG_DIR, 'eval'))
                ...
                #                              eval_freq=max(PPO_N_STEPS * 5, 10000), # 每 N 步評估一次
                #                              n_eval_episodes=5, # 評估 5 個回合
